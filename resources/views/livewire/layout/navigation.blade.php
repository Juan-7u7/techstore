        ...
        <div class="flex justify-between h-16">
            ...
            @auth
                ...
                <div class="hidden sm:flex sm:items-center sm:ms-6">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center gap-2 px-4 py-2 border border-accent/20 text-sm font-medium rounded-full text-white bg-primary/20 hover:bg-primary/30 focus:outline-none transition">
            ...
            @else
                ...
                <div class="hidden sm:flex sm:items-center sm:ms-6 gap-3">
                    ...
                    <a href="{{ route('register') }}" wire:navigate class="px-4 py-2 text-sm font-medium rounded-full bg-primary text-white hover:bg-primary/80 transition">Registrarse</a>
                </div>
            @endauth

            <!-- Boton hamburguesa responsive -->
            <div class="flex items-center sm:hidden">
                <button @click="open = ! open" class="inline-flex items-center justify-center p-2 rounded-full text-accent hover:bg-white/10 focus:outline-none transition">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-dark/95 border-t border-white/10">
        ...
        @else
            <div class="pt-4 pb-3 border-t border-white/10 px-4 space-y-2">
                ...
                <a href="{{ route('register') }}" wire:navigate class="block px-3 py-2 text-sm font-medium text-center rounded-full bg-primary text-white hover:bg-primary/80 transition">Registrarse</a>
            </div>
        @endauth
    </div>
</nav>
