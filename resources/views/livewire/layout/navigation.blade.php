...
<nav x-data="{ open: false }" class="bg-surface border-b border-border/50 sticky top-0 z-50 backdrop-blur-lg bg-surface/90"
    ...
    <template x-teleport="body">
        <div x-show="open" class="relative sm:hidden z-50" x-cloak>
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="translate-x-full" x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" class="fixed top-0 right-0 bottom-0 w-[80vw] max-w-xs bg-surface border-l border-border/50 flex flex-col overflow-y-auto">
                <div class="flex items-center justify-between px-4 py-3 border-b border-border/30">
                    <a href="{{ route('productos.index') }}" wire:navigate class="flex items-center gap-2.5" @click="open = false">
                        <img src="{{ asset('images/logo.svg') }}" alt="TechStore" class="h-6 w-auto">
                        <span class="font-heading font-semibold text-primary text-base tracking-tight">TechStore</span>
                    </a>
                    <button @click="open = false" class="p-1.5 rounded-lg text-muted hover:bg-border/50 transition-colors">
                ...
                <div class="flex-1 px-3 py-4 space-y-0.5">
                    <a href="{{ route('productos.index') }}" wire:navigate class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('productos.*') ? 'text-primary bg-primary/5' : 'text-muted hover:text-primary hover:bg-fondo' }} transition-colors" @click="open = false">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                        </svg>
                        Productos
                    </a>
                    <a href="{{ route('dashboard') }}" wire:navigate class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg {{ request()->routeIs('dashboard') ? 'text-primary bg-primary/5' : 'text-muted hover:text-primary hover:bg-fondo' }} transition-colors" @click="open = false">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                ...
                <div class="border-t border-border/30 px-3 py-4">
                    @auth
                        <div class="flex items-center gap-3 mb-3">
                            <span class="w-9 h-9 rounded-full bg-accent/20 flex items-center justify-center text-sm font-semibold text-accent shrink-0">
                        ...
                        <div class="space-y-0.5">
                            <a href="{{ route('profile') }}" wire:navigate class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-muted hover:text-primary hover:bg-fondo rounded-lg transition-colors" @click="open = false">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z"/>
                                </svg>
                                Mi perfil
                            </a>
                            <button wire:click="logout" class="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-muted hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
